...
// 구조 : Root > section.vidbox > video box(>video+img) + title box(>heading+p tag)
export function VidIntro(props) {
  ...
  // 링크 생성함수 : desc 데이터 / sum 데이터에서 처리
  const linkCode = (data)=>{
    
    return<>
      {data.split('*')[0]} <a href={selData.link[1]} target='_blank'>{selData.link[0]}</a> {data.split('*')[1]}
    </>
  }; ///////// linkCode 함수 /////////////

  return (
    // props.cls로 전달되는 값을 클래스로 넣기
    <section className={"vidbox "+props.cls}>
      <div>
        ...
        <div className="vb2">
          ...
          {/* 요약소개(링크포함):sum */}
          <p>
            {/* 특수문자(*)여부에 따라 처리 indexOf(문자열) : 없으면 -1리턴 */}
            {
