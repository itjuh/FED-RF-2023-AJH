...
***********************************/
function goR1(){
    // console.log(autoI); //undefined=>false, 12345=>true
    if(!autoI){ //처음 누를때만 들어감
        //인터발 실행하기
        autoI = setInterval(()=>{
            r1.style.left = ++r1Pos + 'px';
            whoWinner();
        },level.value);
        // 실행시간은 #level의 값을 읽어온다
    }
...
function goGame(){
    ...
    if(btxt =='토끼출발'){
        ...
        // console.log(r1Pos,t1Pos);

        // 거북버튼 키보드 작동 막기
        // focus가 가는 것을 블러처리하면 된다
        this.blur();
        // 초점 가게하는 메서드 -> focus();
        // 초점 빠지게 하는 메서지 -> blur();
        // 토끼 출발 호출
...
function whoWinner(){
    ...
    // 기준값 : 650px
    if (t1Pos >= 650 || r1Pos >=650){
        // (1) 토끼야 멈춰라
        clearInterval(autoI);
        // (2) 거북아 멈춰라
        t1Stop = 1; // 거북 멈춤변수 상태값 업데이트
        // (3) 승자판별
        // 승자변수 
        let winner;
