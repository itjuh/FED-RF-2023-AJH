...
///// JSX를 사용한 것과 JSX를 사용하지 않은 것을 비교 /////
// 1. JSC를 사용한 예 /////////////////////////////////
// (1-1) 넣을 요소 만들기
const myEle1 = <h1>나는 JSX를 사용하고 있어!</h1>;
// (1-2) 리액트 루트 선택하기 : creatRoot() 메서드 사용!
const root1 = ReactDOM.createRoot(document.querySelectorAll('#root>div')[0]);
// (1-3) 요소 출력하기 : 생성 된 루트에 render()메서드를 붙여서 사용
root1.render(myEle1);

// 2. JSX를 사용하지 않는 방법 ////////////////////////
// -> 넣을 요소를 createElement()메서드로 생성해야 함(JSX안씀)
...
// 표현식에 쓸 변수 : 조건문No, 제어문No, 삼항연산자 OK 바로 보이는 값
...
// 출력형식: map()만 사용해도 바로 출력가능
// 출력문
const myEle5 = (
    <React.Fragment>
        <h2>[ 배우리스트 ]</h2>
        <ul>
            {mydata.map(val=><li>{val.name} : {val.movie} : {val.idx==3?"아름다워💖":"멋져✨"}</li>)}
        </ul>
    </React.Fragment>
);
// 다섯번째에 출력하기
...
const myEle7 = <h1 className="myclass">className 속성으로 클래스를 세팅한다!</h1>;
// 일곱번째에 출력하기
...
let t = new Date;
let time = t.getMinutes();

const myEle9 =(
    <React.Fragment>
        <h1>지금 몇시지? {time}시야!</h1>
        <h1>{time>9?"지금 집에 바로 가야겠어😱":"더 놀아야지~😙"}</h1>
    </React.Fragment>
);
// 아홉번째에 출력하기
