...
export function MySwiper() {
  ...
  const move = (e) => {
    pos -= 25;
    // 대상요소 움직이기
    $(".slider-box").animate({ top: pos + "%" }, 700, "easeInOutQuad");
    ...
    if (!isFirst) {
      ...
      $(".slider")
        .first()
        .animate({ top: -pos + "%" }, 700, "easeInOutQuad");
    } else {
      // 두번째 대상인 경우
      $(".slider")
        .first()
        .animate({ top: -pos + "%" }, 700, "easeInOutQuad");
      $(".slide").eq(2).addClass("on");
      pos = -25;
      $(".slider")
        .eq(2)
        .animate({ top: -pos + "%" }, 700, "easeInOutQuad");
      $(".slider-box").css({ top: -pos + "%" });
      $(".slider-box").append($(".slider").first());
  ...
  return (
    <div className="slider-area row-12">
